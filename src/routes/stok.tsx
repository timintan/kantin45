...
function UpdateStok() {
  ...
  const filteredProducts = useMemo(() => {
    if (!searchProduk.trim()) return products;
    const q = searchProduk.toLowerCase().trim();
    return products.filter(p =>
      p.nama.toLowerCase().includes(q) ||
      (p.barcode && p.barcode.toLowerCase().includes(q)) ||
      (p.kategori && p.kategori.toLowerCase().includes(q))
    );
  }, [products, searchProduk]);
    ...
    <div className="flex flex-col overflow-y-auto h-full">
          ...
          <CardHeader>
            ...
            <div className="relative mt-2">
              ...
              <Input
                ...
                className="pl-9 border-[3px] border-primary focus-visible:ring-primary"
              />
            </div>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-[13px]">
                ...
                <tbody>
                  {filteredProducts.length === 0 ? (
                    <tr><td colSpan={7} className="text-center text-muted-foreground p-5">{searchProduk ? 'Tidak ditemukan' : 'Belum ada produk'}</td></tr>
                    ...
                    <tr key={p.id} className="border-b hover:bg-muted/50 transition-colors">
                      ...
                      <td className="p-2.5">
                        <div className="flex gap-1.5">
                          <button onClick={() => openEdit(p)} className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-primary/10 text-primary transition-colors">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Nama Produk *</Label><Input value={pNama} onChange={e => setPNama(e.target.value)} /></div>
            <div className="space-y-1"><Label>Barcode</Label><Input value={pBarcode} onChange={e => setPBarcode(e.target.value)} /></div>
            <div className="space-y-1"><Label>Kategori</Label><Input value={pKategori} onChange={e => setPKategori(e.target.value)} placeholder="Makanan / Minuman / dll" /></div>
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1"><Label>Stok</Label><Input type="number" value={pStok} onChange={e => setPStok(e.target.value)} /></div>
              <div className="space-y-1"><Label>Harga Beli</Label><Input type="number" value={pHargaBeli} onChange={e => setPHargaBeli(e.target.value)} /></div>
              <div className="space-y-1"><Label>Harga Jual</Label><Input type="number" value={pHargaJual} onChange={e => setPHargaJual(e.target.value)} /></div>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Stok</Label><Input type="number" value={epStok} onChange={e => setEpStok(e.target.value)} /></div>
            <div className="space-y-1"><Label>Harga Beli</Label><Input type="number" value={epHargaBeli} onChange={e => setEpHargaBeli(e.target.value)} /></div>
