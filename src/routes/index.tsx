...
import { ShoppingCart, Trash2, CreditCard, QrCode, Printer, ChevronsUpDown, Check, Wine, Croissant, Package, Camera } from 'lucide-react';
...
function printStruk(trx: { items: { nama: string; qty: number; subtotal: number }[]; total: number }) {
  ...
  win.document.write(`<html><head><title>Struk</title><style>body{font-family:monospace;padding:10px;width:280px}h2{text-align:center;margin-bottom:5px}.center{text-align:center;font-size:12px}table{width:100%;font-size:12px;margin-top:10px}td{padding:3px 0}.line{border-top:1px dashed #000;margin:6px 0}.total{font-weight:bold;font-size:14px}</style></head><body><h2>KANTIN 45</h2><div class="center">Kantin SMPN 45 Semarang</div><div class="line"></div><table>${itemsHtml}</table><div class="line"></div><table><tr><td>Total</td><td style="text-align:right" class="total">${formatRp(trx.total)}</td></tr></table><div class="line"></div><div class="center">Terima kasih 🙏</div><scr` + `ipt>window.onload=function(){window.print();window.onafterprint=function(){window.close()}}</scr` + `ipt></body></html>`);
  win.document.close();
}
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0">
        <div className="mb-3">
          <label className="text-xs text-muted-foreground block mb-1 font-bold">Anggota (opsional)</label>
...
function Minimarket() {
  ...
  const processBarcode = useCallback((code: string) => {
    ...
    const prod = products.find(p => normalizeBarcode(p.barcode) === normalizedCode);
    if (prod) {
      ...
      toast.error(`Barcode ${normalizedCode} tidak ditemukan`);
  ...
  return (
    <div className="flex h-full overflow-hidden relative">
      <div className="flex-1 flex flex-col p-4 sm:p-5 sm:pl-7 overflow-hidden">
        <h1 className="text-lg sm:text-xl font-bold mb-3">Kasir</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-2 sm:gap-3 overflow-y-auto flex-1 pr-1 content-start pb-16 md:pb-0">
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.map(p => (
              ...
              >
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary/10 rounded-[10px] flex items-center justify-center mx-auto mb-2">
                  {p.kategori?.toLowerCase() === 'minuman' ? <Wine className="w-5 h-5 sm:w-6 sm:h-6 text-primary" /> : p.kategori?.toLowerCase() === 'makanan' ? <Croissant className="w-5 h-5 sm:w-6 sm:h-6 text-primary" /> : <Package className="w-5 h-5 sm:w-6 sm:h-6 text-primary" />}
                </div>
                <div
      ...
      <Dialog open={qrisOpen} onOpenChange={setQrisOpen}>
        <DialogContent className="sm:max-w-md max-h-[95vh] flex flex-col p-4 sm:p-6 gap-2">
          ...
          <div className="text-center flex-1 min-h-0 overflow-y-auto">
            ...
            <p className="text-xs text-muted-foreground mb-2">Scan QRIS di bawah ini untuk membayar</p>
            <div className="bg-muted rounded-xl p-3 inline-block mb-2">
              <div className="w-[200px] h-[200px] bg-muted-foreground/10 rounded-lg flex items-center justify-center text-muted-foreground text-sm">
                QRIS
              </div>
            </div>
          </div>
          <DialogFooter className="flex-col gap-2 sm:flex-col">
            <Button onClick={confirmQris} className="w-full gap-2">
              <CreditCard className="h-4 w-4" /> Konfirmasi Pembayaran
            </Button>
            <Button variant="outline" onClick={() => { printStruk(pendingStruk); }} className="w-full gap-2">
